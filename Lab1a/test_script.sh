#!/bin/bash
TMP_DIR=/tmp/p1a_tmp
REQUIRED_FILES="readme.txt team.txt calculator.php"
...
if [ ! -f ${ZIP_FILE} ]; then
    echo "ERROR: Cannot find ${ZIP_FILE}, ensure this script is put in the same directory of your ${ZIP_FILE} file. Otherwise check the zip file name" 1>&2
    rm -rf ${TMP_DIR}
    echo "rmd"
    exit 1
fi
