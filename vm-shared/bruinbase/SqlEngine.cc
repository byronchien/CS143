    ...
    for (int k = 0; k < key_cond.size(); k++) {
      switch(key_cond[k].comp) {
        ...
        case SelCond::NE:
          ...
          break;
        case SelCond::GT:
          lower_limit = (*key_cond[k].value > lower_limit) ? *key_cond[k].value + 1
            : lower_limit;
          break;
        case SelCond::LT:
          upper_limit = (*key_cond[k].value < upper_limit) ? *key_cond[k].value + 1
            : upper_limit;
          break;
        case SelCond::GE:
          lower_limit = (*key_cond[k].value > lower_limit) ? *key_cond[k].value
            : lower_limit;
          break;
        case SelCond::LE:
          upper_limit = (*key_cond[k].value < upper_limit) ? *key_cond[k].value
            : upper_limit;
          break;
      }
    }
